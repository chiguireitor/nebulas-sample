...
export(String) var send_to_address
export(String) var send_amount
export(bool) var send setget _send_set, _send_get
...
func _send_set(v):
	if v:
		var prnt = get_parent()
		var tx = prnt.find_transaction_object()
		create_send(tx, send_to_address, send_amount)
		...
		if result:
			if tx.send_tx(1):
				...
				if tx_result.has("result") and tx_result.result.has("result"):
					...
					_current_warning = "Error while broadcasting transaction"
			else:
				_current_warning = "Couldn't send tokens"
		else:
			_current_warning = "Couldn't estimate gas"
			
		if len(_current_warning) > 0:
			update_configuration_warning()
